			...
			public void actionPerformed(ActionEvent e) {
				...
				for(int i=0;i <table.getRowCount();i++)
				{
					if((boolean) table.getValueAt(i, 6))
					{
						...
						index.deleteLine(fold, destination,(emi[(10*((ap.list.size()/10)-page+1)+(ap.list.size()%10)-i-1)].getPath()));
					}
				}
			...
			public void actionPerformed(ActionEvent e) {
				...
				for(int i=0;i<table.getRowCount();i++)
				{
					if((boolean) table.getValueAt(i, 6))
					{
						//emi a5rha 10
						//formula lw aktr mn 10 ---> (10*((list.size/10)-page+1)+(list.size%10)-i-1)
						mails.add(((Mail)allSLL.get((10*((ap.list.size()/10)-page+1)+(ap.list.size()%10)-i-1))).getPath());
						allSLL.remove(((10*((ap.list.size()/10)-page+1)+(ap.list.size()%10)-i-1)));
